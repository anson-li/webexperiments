const PerlinMouseFs = `
  ...
  void main() {
    ...
    vec3 cloudEffect = clouds(vertexPosition.x, vertexPosition.y);
    color = cloudEffect + vec3(.95, .8, 0.95);

    float distance = distance(vec2(vertexPosition.x, vertexPosition.y), uMousePosition);
    ...
    float isRevealed = min(distance / 0.5 - 0.2, 1.);
